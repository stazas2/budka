...
function sendImageToServer(imageData) {
  try {
    ...
    fs.writeFileSync(logFilePath, `Headers: ${JSON.stringify(headers, null, 2)}\nData: ${JSON.stringify(data, null, 2)}`, "utf-8");
  ...
  try {
    ...
    if (imagesArray && imagesArray.length > 0) {
      ...
      resultImage.onload = () => {
        ...
        console.log("Image dimensions: ", resultImage.width,"x", resultImage.height);
...
if (printPhotoButton) {
  printPhotoButton.addEventListener("click", () => {
    ...
    if (resultImage && resultImage.src) {
      ...
      ipcRenderer.send("print-photo", { imageData: imageData, isLandscape: isLandscape });
    } else {
